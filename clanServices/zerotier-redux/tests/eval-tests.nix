...
{
  test_simple = {
    inherit evaluatedService testFlake;

    expr = {
      enabledViaInventory = {
        jon = testFlake.nixosConfigurations.jon.config.clan.zerotier.enabledViaInventory or false;
        sara = testFlake.nixosConfigurations.sara.config.clan.zerotier.enabledViaInventory or false;
      };
      instanceNames = lib.attrNames evaluatedService.instances;
    };
    expected = {
      enabledViaInventory = {
        jon = true;
